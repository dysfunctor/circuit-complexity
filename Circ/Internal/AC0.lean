...
used to prove that AC0 implies AC0NFTree.
...
* `InAC0_implies_InAC0NFTree` — AC0 ⊆ AC0NFTree
...
theorem dedup_wireDepth_le (c : Circuit Basis.unboundedAON N 1 G) (w : Fin (N + G)) :
  ...
  by_cases hw : w.val < N
  · simp [inputWireDepth, hw]
  · have hG : w.val - N < G := by omega
    rw [gateWireDepth c.dedup w hw, gateWireDepth c w hw]
    ...
    calc c.dedup.wireDepth ((c.gates ⟨w.val - N, hG⟩).dedup.inputs k)
      ...
      _ ≤ Fin.foldl _ (fun acc k => max acc (c.wireDepth ((c.gates ⟨w.val - N, hG⟩).inputs k))) 0 :=
          foldl_max_le_elem _ (fun k => c.wireDepth ((c.gates ⟨w.val - N, hG⟩).inputs k)) k'
termination_by w.val
decreasing_by
...
theorem dedup_depth_le (c : Circuit Basis.unboundedAON N 1 G) :
  ...
  simp only [Circuit.depth, outputDepth, Fin.foldl_succ, Fin.foldl_zero, Nat.zero_max]
  apply Nat.add_le_add_left
  apply foldl_max_le_bound
  intro k
  ...
  calc c.dedup.wireDepth ((c.outputs 0).dedup.inputs k)
    ...
    _ ≤ _ := foldl_max_le_elem _ (fun k => c.wireDepth ((c.outputs 0).inputs k)) k'
...
private def acnfOneBit (f : BitString 1 → Bool) : (b : Bool) × ACNFTree 1 b :=
...
private theorem acnfOneBit_eval (f : BitString 1 → Bool) :
  ...
  cases hT : f (fun _ => true) <;> cases hF : f (fun _ => false) <;>
    simp [ACNFTree.eval, ACNFTree.eval.evalAll, ACNFTree.eval.evalAny, Literal.eval] <;>
    cases x 0 <;> simp_all
...
private theorem acnfOneBit_depth_le (f : BitString 1 → Bool) :
  ...
  cases f (fun _ => true) <;> cases f (fun _ => false) <;>
    simp [ACNFTree.depth, ACNFTree.depth.depthList]

private theorem acnfOneBit_leafCount_le (f : BitString 1 → Bool) :
    (acnfOneBit f).2.leafCount ≤ 1 := by
  simp only [acnfOneBit]
  cases f (fun _ => true) <;> cases f (fun _ => false) <;>
    simp [ACNFTree.leafCount, ACNFTree.leafCount.leafCountList]

/-- Every AC0 function family is also in AC0NFTree. -/
theorem InAC0_implies_InAC0NFTree (f : BoolFunFamily) : InAC0 f → InAC0NFTree f := by
  intro ⟨d, c, hf⟩
  ...
  · -- N = 1: construct ACNF directly (circuit approach gives leafCount > 1^c')
    ...
    exact ⟨(acnfOneBit (f 1)).1, (acnfOneBit (f 1)).2,
      Nat.le_trans (acnfOneBit_depth_le _)
        (by have : circ.depth ≤ d := hdepth; simp [Circuit.depth, Circuit.outputDepth, Fin.foldl_succ, Fin.foldl_zero] at this; omega),
  ...
  · -- N ≥ 2: dedup the circuit, then convert to ACNF
    ...
    use circ'.toACNFTree.1, circ'.toACNFTree.2
    refine ⟨?depth, ?leafCount, ?eval⟩
    case eval =>
      funext x
      rw [Circuit.toACNFTree_eval, congr_fun (congr_fun circ.dedup_eval x) 0]
      exact congr_fun heval x
    case depth =>
      exact Nat.le_trans (Circuit.toACNFTree_depth_le circ') (Nat.le_trans circ.dedup_depth_le hdepth)
    case leafCount =>
      calc circ'.toACNFTree.2.leafCount
          ≤ circ'.maxFanIn ^ circ'.depth := Circuit.toACNFTree_leafCount_le circ'
...
/-- Contrapositive: a function not in AC0NFTree is not in AC0. -/
theorem not_InAC0NFTree_implies_not_InAC0 (f : BoolFunFamily) : ¬InAC0NFTree f → ¬InAC0 f :=
  fun h => h ∘ InAC0_implies_InAC0NFTree f
